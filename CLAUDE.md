...
Hermes Gateway (run.py, feishu.py)
    ↕ HTTP POST /events, /card_action
Sidecar (aiohttp, :8765)
    ├── server.py          — HTTP 路由 (/events, /card_action, /alerts, /health)
    ├── session.py         — CardSession 状态机
    ├── render.py          — 卡片渲染 (CardKit 模式 + Legacy 模式)
    ├── cardkit.py         — [新增] CardKit v2.0 卡片构建
    ├── card_action.py     — [新增] 交互帮助卡片按钮回调
...
三大能力：CardKit v2.0 流式渲染（降级 IM PATCH）、交互帮助卡片、多 Profile/Bot。
...
## 协作规则 (Hermes)
- 完成任务后用 Node.js 发飞书消息通知 Hermes (bot open_id: ou_bcb48e73ca7890a12ac93b588437167b)。
- Hermes API: http://127.0.0.1:8642（OpenAI 兼容格式）。
- 消息内容: 项目路径、GitHub URL、commit hashes、变更摘要、需要做的事。
